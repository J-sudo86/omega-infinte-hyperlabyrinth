...
class Projectile:
    def __init__(self,x,y,angle,speed,damage,big=False):
        ...
        self.radius=5 if not big else 8
    @property
    ...
    def update(self,dt,walls,enemies):
        ...
        self.x,self.y=new
        for e in enemies:
            if e.alive and math.hypot(self.x-e.x,self.y-e.y)<self.radius+e.radius:
                e.hit(self.damage);self.alive=False
                return ('shrapnel',self.x,self.y) if not self.big else ('explosion',self.x,self.y)
        if not (0<=self.x<=WORLD_W and 0<=self.y<=WORLD_H):self.alive=False
        return None
    def draw(self,s):
        if self.alive:pygame.draw.circle(s,ORANGE,(int(self.x),int(self.y)),self.radius)
